...
namespace D007.建立和起始有參數非同步工作
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // 建立完成後，該非同步工作，就會自動啟動
            ThreadPool.QueueUserWorkItem(new WaitCallback(我的非同步方法), Guid.NewGuid());

            // 方法2 : 使用 Task 類別，在建立物件的時候，傳入非同步的委派方法   .NET 4 開始提供
            // 產生完 Task 物件之後，需要呼叫 Start() 方法，來啟動這個非同步委派方法
            var t1 = new Task(我的非同步方法, Guid.NewGuid());
            t1.Start();
            var t2 = new Task(() => 我的非同步方法(Guid.NewGuid()));
            t2.Start();
            ...
            Task.Factory.StartNew(我的非同步方法, Guid.NewGuid());
            Task.Factory.StartNew(() => 我的非同步方法(Guid.NewGuid()));
            ...
            Task.Run(() => 我的非同步方法(Guid.NewGuid()));

            Console.WriteLine("按下任一按鍵，結束處理程序");
            Console.ReadKey();
        }

        static void 我的非同步方法(object state)
        {
